...
- `gpu_image_suffix`: Image suffix appended to the image tag when `enable_gpu` is `true`, selecting which published GPU image variant to pull. Appended to **both** the Docker Hub default tag and a caller-supplied `image_ref` template, so it is a single source of truth for the CUDA variant. The image actually pulled becomes `picknikciuser/moveit-studio:<image_tag>-<ros_distro><gpu_image_suffix>` (default path) or `<image_ref-formatted><gpu_image_suffix>`. Defaults to `-cuda12.6-cudnn9` for backward compatibility; pass `-cuda13.2-cudnn9` for Jazzy/CUDA 13.2 images (MoveIt Pro 9.4.0+ publishes Jazzy GPU images only under the `-cuda13.2-cudnn9` suffix). Ignored only when `enable_gpu` is `false`. Default: `-cuda12.6-cudnn9`.
- `image_ref`: Full container image reference template that overrides the default Docker Hub `picknikciuser/moveit-studio:<image_tag>-<ros_distro>`. Used by sister repos that need to pull a PR-built private GHCR image via `repository_dispatch`. Uses Python-style `format()` semantics: `{0}` is substituted with the matrix ROS distro at job start, so one input covers every distro the matrix expands over. Example: `ghcr.io/picknikrobotics/moveit-studio:my-branch-{0}-amd64`. When set and `enable_gpu` is `true`, `gpu_image_suffix` IS appended to the formatted ref; pass a suffix-free template and let the workflow add the CUDA suffix. Default: `""` (use Docker Hub via `image_tag`).
