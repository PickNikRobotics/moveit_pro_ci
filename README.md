...
- `gpu_image_suffix`: Image suffix appended to the Docker Hub tag when `enable_gpu` is `true` (and `image_ref` is unset), selecting which published GPU image variant to pull. The image actually pulled becomes `picknikciuser/moveit-studio:<image_tag>-<ros_distro><gpu_image_suffix>`. Defaults to `-cuda12.6-cudnn9` for backward compatibility; pass `-cuda13.2-cudnn9` for Jazzy/CUDA 13.2 images (MoveIt Pro 9.4.0+ publishes Jazzy GPU images only under the `-cuda13.2-cudnn9` suffix). Ignored when `enable_gpu` is `false` or `image_ref` is set. Default: `-cuda12.6-cudnn9`.
