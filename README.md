...
- `lfs_cache_s3_bucket`: If set, Git LFS objects are fetched through an S3-backed cache instead of a direct pull: objects are restored from this bucket, any still-missing objects are pulled from the Git remote, and the resulting set is saved back for future runs (keyed by the workspace's LFS object set). Leave empty for a normal direct LFS pull. Requires the `lfs_cache_role_arn` secret, and the calling job must grant `id-token: write` permission. Default: `""`.
